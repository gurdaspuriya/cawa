...
def compute_sov(b, tlbl):
	...
	for i in range(len(tlbl)):
		...
		sov.append(sov_temp/len(ground_truth));
		sov_smooth.append(sov_smooth_temp/len(ground_truth));
		accuracy.append(float(sum([1 for j in range(len(ground_truth)) if ground_truth[j]==predicted[j]]))/len(ground_truth));
	return (sov, sov_smooth, accuracy, overload_labels, overload_segments);
